      ...
      <div className="flex gap-2 mt-2">
        {testimonials.map((_, i) => (
          <button
            key={i}
            onClick={() => setIdx(i)}
            className={`w-3 h-3 rounded-full ${i === idx ? "bg-[#BC1782]" : "bg-gray-300"
              } transition`}
      ...
      >
        {donateOptions.map((item) => (
          <motion.div
            ...
            whileTap={{ scale: 0.97 }}
            className={`bg-white p-4 xs:p-6 md:p-8 rounded-3xl shadow-xl transition text-center relative border-2 ${item.highlight ? "border-yellow-300" : "border-pink-100"
            ...
            <div className="flex flex-col flex-grow justify-between h-full">
              ...
              <div className="mt-auto">
                ...
                >
                  {item.custom
                    ? "Donate Custom"
                    : `Donate ₹${item.amount ? item.amount.toLocaleString() : "Custom"
                    }`}
                </button>
              </div>
            </div>
