...
const DonateForACause = () => {
  ...
  const handleDonateClick = (donationOption) => {
    ...
    // Map display-friendly title to server-compatible purpose
    const purposeMap = {
      Sponsor: "Sitaare Sponsor",
      "Support Daily Nutrition": "Sitaare Nutrition",
      "Adopt a Sitaare for a Month": "Sitaare Full Care",
      "One-Time Meal (All Sitaare)": "Sitaare One-Time Meal",
      "Meal for 2 Girls (Full Day)": "Sitaare Meal for Two",
      "Sponsor a Celebration": "Sitaare Celebration",
      "General Donation": "General Donation",
    };

    const title = donationOption.title?.trim();
    const purpose =
      title && purposeMap[title] ? purposeMap[title] : "General Donation"; // Fallback to 'General Donation'

    // Construct query parameters
    const queryParams = new URLSearchParams({
      amount: donationOption.amount.toString(),
      purpose: encodeURIComponent(purpose), // Ensure proper URL encoding
    });

    // Use production URL in production, development URL in dev
    const baseUrl =
      process.env.REACT_APP_HOH_BASE_URL || "http://localhost:5173";
    const url = `${baseUrl}/donate-for-a-cause?${queryParams.toString()}`;
    console.log("Redirecting to donation URL:", url);
    console.log("Donation option:", donationOption);
  ...
  const handleRedirectToDonate = () => {
    // Redirect to HOH donation page without prefilled values
    window.open("http://localhost:5173/donate-for-a-cause", "_blank");
    setShowCelebration(false);
  };
